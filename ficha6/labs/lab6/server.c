...
int main(int argc, char *argv[]){
    ...
    if (close(udp_server_socket) == -1){
	    ERROR(C_ERR_CANT_CLOSE_SOCKET,
			    "Can't close udp_server_socket (IPv4)");
    }
...
int handle_client(int serv_sock){
	...
	ssize_t ret_recvfrom = recvfrom(serv_sock, &clnt_value,
	   sizeof(clnt_value), 0, (struct sockaddr*)&clnt_addr,&clnt_addr_len);
	if( ret_recvfrom == -1 ){
	...
	printf("DEBUG:sqrt_clnt='%s'\n", S);

	ssize_t ret_sendto = sendto(serv_sock,S,strlen(S), 0, 
